...
function applyWorkspaceFileOperations(text, cwd, options = {}) {
    ...
    for (const operation of operations) {
        ...
        try {
            ...
            const body = String(operation.content || '').trimStart();
...
function buildWorkspaceChatContext(cwd, prompt = '', historyMessages = []) {
    ...
    for (const file of files) {
        ...
        const mentioned = promptText.includes(file.relativePath.toLowerCase()) || promptText.includes(path.basename(file.relativePath).toLowerCase());
        if (!mentioned && total > 0 && total + file.bytes > MAX_CONTEXT_TOTAL_BYTES / 2) continue;
