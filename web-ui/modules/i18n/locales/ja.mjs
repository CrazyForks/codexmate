const ja = Object.freeze({
    ...
    'orchestration.hero.subtitle': 'タスクメッセージを送り、計画をプレビューしてから実行します。',
    'orchestration.quick.kicker': 'クイックタスク',
    'orchestration.quick.title': 'Codexmate に何を完了させますか？',
    'orchestration.quick.subtitle': 'まずゴールを書くだけで、計画をプレビューしてから実行します。詳細設定は必要になるまで隠します。',
    'orchestration.quick.caption': '中央のメインボタンで実行します。依頼だけ追加する場合は Enter、計画だけ確認する場合は /plan を送信します。長い作業や一括処理は詳細設定のキューを使います。',
    ...
    'orchestration.chat.input.placeholder': 'Codexmate に完了してほしいことを直接書いてください。例: 現在の PR レビューコメントを修正し、回帰テストを追加し、検証スクリーンショットを添付する',
    'orchestration.chat.input.hint': 'チャットのように要件を書くだけで、workspace・thread・run mode は既定で自動推定され、必要な時だけ Advanced に表示されます。',
    ...
    'orchestration.chat.input.send': '依頼を送信',
    'orchestration.chat.input.discuss': '計画を確認',
    ...
    'orchestration.chat.input.workHint': '実行開始は現在の入力を先に取り込みます。',
    'orchestration.chat.input.workCaption': 'ボタンは実行開始の 1 つだけです。Enter で依頼を追加し、/plan で実行せず計画だけ確認できます。',
    ...
    'orchestration.readiness.empty.title': 'まずタスクメッセージを書く',
    'orchestration.readiness.empty.summary': '達成したい結果をチャットのように明確にしてから、オーケストレーターにノード分解させます。',
    ...
    'orchestration.readiness.preview.summary': '下書きはできています。チャットで /plan を送信し、ノードと依存関係を確認してください。',
    ...
    'orchestration.stage.subtitle': 'まずタスクメッセージを書き、計画をプレビューします。',
