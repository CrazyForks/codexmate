const zh = Object.freeze({
    ...
    'orchestration.hero.title': 'Web Agent 工作台',
    'orchestration.hero.subtitle': '在一个 tab 内讨论方案、启动 agent、追踪执行状态和恢复上下文。',
    'orchestration.quick.kicker': 'AGENT CONSOLE',
    'orchestration.quick.title': '让 Codexmate 像 agent 一样接任务、规划并执行',
    'orchestration.quick.subtitle': '先讨论方案，再按明确授权启动执行；workspace、thread、records 会在同一个 cockpit 里保留。',
    'orchestration.quick.caption': '探讨方案只生成计划；开始执行会吸收当前输入、自动规划并启动 agent。批量入队仍放在高级设置里。',
    ...
    'orchestration.chat.input.placeholder': '告诉 Codexmate 要完成什么；发送 /plan 可把当前需求转成执行方案。',
    'orchestration.chat.input.hint': '像聊天一样写需求；用 /plan 触发方案起草，workspace、thread 和运行模式需要时再进高级设置。',
    ...
    'orchestration.chat.input.send': '发送需求',
    'orchestration.chat.input.discuss': '探讨方案',
    'orchestration.chat.input.work': '开始执行',
    'orchestration.chat.input.firstHint': '这会成为需求 1。',
    'orchestration.chat.input.workHint': '探讨方案或开始执行都会先吸收当前输入。',
    'orchestration.chat.input.workCaption': '发送需求只追加到对话；探讨方案只生成计划；开始执行会规划并让 Web Agent 真正跑任务。',
    ...
    'orchestration.chat.assistant.empty': '先发第一个需求；如果还有第二个需求，继续发送，Codexmate 会按顺序处理并保留上下文。',
    ...
    'orchestration.chat.assistant.singleReady': '已收到第一条需求。可以继续发送需求 2，或发送 /plan 起草执行方案。',
    ...
    'orchestration.chat.meta.previewNext': '下一步：发送 /plan',
    'orchestration.quick.checklist.title': '发布前检查',
    'orchestration.quick.checklist.subtitle': '只保留真正影响执行的阻塞项。',
    'orchestration.agent.aria': 'Web Agent 工作台状态',
    'orchestration.agent.kicker': 'WEB AGENT COCKPIT',
    'orchestration.agent.title': 'Agent 正在这个 tab 内接管任务链路',
    'orchestration.agent.subtitle': '左侧选 workspace/session，中间讨论与执行，右侧保留 live trace、run detail 和高级边界。',
    ...
    'orchestration.workspace.newSession': '+ 新会话',
    'orchestration.workspace.sessions.title': 'Session Inbox',
    'orchestration.workspace.sessions.empty.title': '当前项目暂无记录',
    'orchestration.workspace.sessions.empty.subtitle': '先新建会话，或刷新后选择已有 runs。',
    ...
    'orchestration.readiness.target.label': '任务消息',
    'orchestration.readiness.target.done': '已写任务消息',
    'orchestration.readiness.target.missing': '还没写任务消息',
    ...
    'orchestration.readiness.empty.title': '先写任务消息',
    'orchestration.readiness.empty.summary': '先像聊天一样写清楚想完成的结果，再让编排器拆节点。',
    ...
    'orchestration.readiness.preview.summary': '草稿已成形，发送 /plan 预览方案，确认节点和依赖再执行。',
    ...
    'orchestration.stage.subtitle': '先发任务消息，再预览计划。',
    ...
    'orchestration.result.title': '执行回执',
    'orchestration.result.target': '执行目标',
    'orchestration.result.receipt': '执行结果',
    'orchestration.result.summary': '运行摘要',
    'orchestration.result.completed': '已完成：{target}',
