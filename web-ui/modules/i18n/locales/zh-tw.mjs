const zhTw = Object.freeze({
    ...
    'orchestration.hero.subtitle': '在一個 tab 內討論方案、啟動 agent、追蹤執行狀態和恢復上下文。',
    'orchestration.quick.kicker': 'AGENT CONSOLE',
    'orchestration.quick.title': '讓 Codexmate 像 agent 一樣接任務、規劃並執行',
    'orchestration.quick.subtitle': '先討論方案，再按明確授權啟動執行；workspace、thread、records 會在同一個 cockpit 裡保留。',
    'orchestration.quick.caption': '探討方案只產生計畫；開始執行會吸收目前輸入、自動規劃並啟動 agent。批次入列仍放在進階設定裡。',
    ...
    'orchestration.chat.input.placeholder': '告訴 Codexmate 要完成什麼；送出 /plan 可把目前需求轉成執行方案。',
    'orchestration.chat.input.hint': '像聊天一樣寫需求；用 /plan 觸發方案起草，workspace、thread 和執行模式需要時再進進階設定。',
    ...
    'orchestration.chat.input.send': '送出需求',
    'orchestration.chat.input.discuss': '探討方案',
    'orchestration.chat.input.work': '開始執行',
    'orchestration.chat.input.firstHint': '這會成為需求 1。',
    'orchestration.chat.input.workHint': '探討方案或開始執行都會先吸收目前輸入。',
    'orchestration.chat.input.workCaption': '送出需求只追加到對話；探討方案只產生計畫；開始執行會規劃並讓 Web Agent 真正跑任務。',
    ...
    'orchestration.chat.assistant.empty': '先送出第一個需求；如果還有第二個需求，繼續送出，Codexmate 會按順序處理並保留上下文。',
    ...
    'orchestration.chat.assistant.singleReady': '已收到第一條需求。可以繼續送出需求 2，或送出 /plan 起草執行方案。',
    ...
    'orchestration.chat.meta.previewNext': '下一步：送出 /plan',
    'orchestration.quick.checklist.title': '發布前檢查',
    'orchestration.quick.checklist.subtitle': '只保留真正影響執行的阻塞項。',
    'orchestration.agent.aria': 'Web Agent 工作台狀態',
    'orchestration.agent.kicker': 'WEB AGENT COCKPIT',
    'orchestration.agent.title': 'Agent 正在這個 tab 內接管任務鏈路',
    'orchestration.agent.subtitle': '左側選 workspace/session，中間討論與執行，右側保留 live trace、run detail 和進階邊界。',
    ...
    'orchestration.workspace.newSession': '+ 新會話',
    'orchestration.workspace.sessions.title': 'Session Inbox',
    'orchestration.workspace.sessions.empty.title': '目前專案暫無記錄',
    'orchestration.workspace.sessions.empty.subtitle': '先新建會話，或重新整理後選擇已有 runs。',
    ...
    'orchestration.readiness.empty.title': '先寫任務訊息',
    'orchestration.readiness.empty.summary': '先像聊天一樣寫清楚想完成的結果，再讓編排器拆節點。',
    ...
    'orchestration.stage.subtitle': '先發任務訊息，再預覽計畫。',
    ...
    'orchestration.result.title': '執行回執',
    'orchestration.result.target': '執行目標',
    'orchestration.result.receipt': '執行結果',
    'orchestration.result.summary': '運行摘要',
